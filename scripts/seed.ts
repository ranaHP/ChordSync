...
async function main() {
  await dbConnect();
  // Backward-compat migration: older builds created a text index that used
  // `language` as Mongo's language override field. That breaks when values
  // like "Sinhala" / "Singlish" are present. Drop that legacy index if it exists.
  try {
    await Song.collection.dropIndex("title_text_singer_text_tags_text");
  } catch {
    // no-op: index may not exist yet
  }
